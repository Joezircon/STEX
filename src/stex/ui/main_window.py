...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.splitter.addWidget(self.forge_workshop)
        ...
        status = QStatusBar()
        status.showMessage(
            "READY  |  INSERT IMAGE TO START  |  ZERO ISLANDS IS THE GOAL"
        )
    ...
    def toggle_forge(self):
        visible = self.forge_workshop.isVisible()
        self.forge_workshop.setVisible(not visible)

        total = max(1, self.splitter.width())
        left = max(220, self.sidebar.width())
